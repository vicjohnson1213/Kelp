...
function interpretFunc(func, env) {
    if (env[func.name]) {
        ...
        if (def.args.length !== func.args.length) {
            return returnError('Wrong parity when calling function: ' + func.name);
        }
    ...
    switch(func.name) {
        ...
        case 'map':
            ...
            if (!_.isArray(list) || closure.type !== 'closure') {
                return returnError('Invalid arguments to (map (list) (lambda))');
            }
        ...
        case 'reduceRight':
            ...
            if (!_.isArray(list) || closure.type !== 'closure') {
                return returnError('Invalid arguments to (reduce (list) (lambda))');
            }
        ...
        default:
            if (!functions[func.name]) {
                return returnError(func.name + ' is not a function');
            }
