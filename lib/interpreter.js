...
function interpretExp(expr, env) {
    switch(expr.type) {
        ...
        case 'string':
            ...
            break;
        case 'closure':
            var newEnv = _.merge({}, env, expr.env);
    ...
    switch(func.name) {
        ...
        case 'map':
            ...
            _.forEach(list, function(el, idx) {
                var newEnv = functions['let'](closure.args[0].value, el, env);

                // Only bind the idx arg if they asked for it.
                if (closure.args[1]) {
                    newEnv = functions['let'](closure.args[1].value, idx, newEnv);
                }

                newList.push(interpretExp(closure, newEnv));
            });
